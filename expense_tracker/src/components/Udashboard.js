...
function Udashboard() {
  ...
  return (
    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} sx={{ backgroundColor: 'primary.dark', color: 'white', height: '100%' }} >
      ...
      <Box sx={{ backgroundColor: 'primary.light', width: { xs: '100%', sm: '20%' }, height: { sm: '100vh', xs: '5rem' } }} >
        <Tabs
          variant="scrollable"
        ...
        >
          ...
          <Link to='/login' ><Tab label="Budgeting" /></Link>
          <Link to='/login' ><Tab label="Expenses Analyser" /></Link>
          <Link to='/login' ><Tab label="Download Expenses Report" /></Link>
