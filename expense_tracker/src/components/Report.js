...
function Report() {
  ...
  const generateReport = () => {
    ...
    html2canvas(input, options).then(async (canvas) => {
      ...
      const pdfFile = new File([pdfBlob], "expense_report.pdf", {
      ...
      Axios.post(
        ...
        .then((res) => {
          ...
          setMonthData({
            ...
            savings_budget: res.data.data.totalBudget,
            cat_distribution: res.data.data.categoryExpenses,
          });
        })
      ...
      >
              ...
              <form onSubmit={handleSubmit}>
                <Stack spacing={2}>
                  <label htmlFor="demo-simple-select">
                    Select how you want to get Report
                  </label>
                  <Select
        ...
        {ViewRepo ? (
          ...
          >
            ...
            {YearData.month_expense.length !== 0 ? (
              <YearReport data={YearData} />
            ) : (
              <></>
            )}
            {MonthData.allExpenses.length !== 0 ? (
              <MonthReport data={MonthData} />
            ) : (
              <></>
            )}
            {GivenRangeData.allExpenses.length !== 0 ? (
              <GivenRangeReport data={GivenRangeData} />
            ) : (
              <></>
            )}
