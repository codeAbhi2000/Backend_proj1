...
function HomeDash() {
  ...
  const getMonthData = async () => {
    ...
    let temp = expense.data.data.filter((e) => {
      ...
      //  console.log(e.date);
      let cdate = e.date.slice(5, 6) === 0 ? e.date.slice(6, 7) : e.date.slice(5, 7)
  ...
  return (
    <>
      ...
      <Stack width='100%' mt={2} direction={{ xs: 'column', sm: 'row' }} alignItems='center' justifyItems='center' justifyContent='space-evenly'>
        {CatData.length > 0 ? <Box width={{ sm: "40%", xs: '80%' }} height={{ sm: '70vh', xs: '55vh' }} sx={{ display: 'flex', flexDirection: 'column' }} alignItems={'center'} justifyContent='center' border='1px solid white' m={1}>
